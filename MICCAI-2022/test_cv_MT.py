...
for k, data in data_cv_splits.items():
	if k > 0:
		...
		print("*******************************************")
		load_path = os.path.join(opt.checkpoints_dir, opt.exp_name, opt.model_name, '%s_%d_best.pt' % (opt.model_name, k))
		...
		# preds_path, feats_path, gt_all = pred_test[5], feats_test[0], feats_test[-1]
