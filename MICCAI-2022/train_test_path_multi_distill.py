...
Author: Xing Xiaohan 
Date: 2022/02/09
Distill knowledge from the pathomic teacher with our proposed DC-Distill and GK-Refine.
"""
...
def train(opt, train_loader, n_data, test_loader, test_loader_patches, device, k):
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter+opt.niter_decay+1)):
        ...
        for batch_idx, ((x_path, ema_x_path), x_grph, x_omic, censor, survtime, \
            ...
            if opt.assign_weights == "True":
                # loss_KD = AEKD_loss_v2(opt, optimizer, loss_cls, path_feat, KD_loss_list)
        ...
        if opt.measure or epoch == (opt.niter+opt.niter_decay - 1):
            ...
            ### test the model.
            if epoch > (opt.niter+opt.niter_decay - 10):
    ...
    return fix_model, model, optimizer, metric_logger
