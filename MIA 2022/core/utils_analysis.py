...
# Glioma
def getGradTestPats_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/grad_15/', model='pathomic_self_MT_5cv_0118', \
    split='test', use_rnaseq=False, agg_type='mean'): #, which_net = "fuse"):

    pats = {}
    ignore_missing_moltype, ignore_missing_histype = 1, 1
    # use_patch, roi_dir, use_vgg_features = ('_patch_', 'all_st_patches_512', 0) if (('path' in model) or ('graph' in model)) else ('_', 'all_st', 0)
    ...
    data_cv_path = './data/TCGA_GBMLGG/splits_5cv_2022/gbmlgg5cv_%s_%d_%d_%d%s.pkl' % (roi_dir, ignore_missing_moltype, ignore_missing_histype, use_vgg_features, use_rnaseq, )
    print(data_cv_path)

    for k in range(1,6):
        pred = pickle.load(open(ckpt_name+'/%s/%s_%d%spred_%s.pkl' % (model, model, k, use_patch, split), 'rb'))   
        print("pred path:", ckpt_name+'/%s/%s_%d%spred_%s.pkl' % (model, model, k, use_patch, split))
        # print(pred) 
        grad_all = pred[5].T
        ...
        print(data_cv_split_k[split]['g'].shape)
        print(pred[8].shape)
        assert np.all(data_cv_split_k[split]['g'] == pred[8]) # Data is correctly registered
        ...
        assert np.all(np.array(all_dataset_regstrd['Grade']) == pred[8])
...
def getPredAggGrad_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/grad_15/', model='pathomic_fusion', split='test', use_rnaseq=False, 
                         agg_type='max', which_net = "fuse", test_pats=getGradTestPats_GBMLGG(), label='all'):
    y_label, y_pred = [], []
    ...
    use_rnaseq = '_rnaseq' if use_rnaseq else ''
    data_cv_path = './data/TCGA_GBMLGG/splits_5cv_2022/gbmlgg5cv_%s_%d_%d_%d%s.pkl' % (roi_dir, ignore_missing_moltype, ignore_missing_histype, use_vgg_features, use_rnaseq, )
    print("data_cv_path:", data_cv_path)
    ...
    for k in range(1,6):
        ...
        pred = pickle.load(open(ckpt_name+'/%s/%s_%d%spred_%s.pkl' % (model, model, k, use_patch, split), 'rb'))  
        # print("pred file:", pred)  
        print("pred file:", ckpt_name+'/%s/%s_%d%spred_%s.pkl' % (model, model, k, use_patch, split))
...
def calcGradMetrics(y_label_all, y_pred_all, avg='micro'):
    ...
    for i in range(5):
        ...
        # print(np.array(y_label).shape, np.array(y_pred).shape)
...
def calcAggGradMetrics(y_label_all, y_pred_all, avg='micro'):
    ...
    for i in range(5):
        ...
        print("split", i, roc_auc_score(y_label, y_pred, average = avg), average_precision_score(y_label, y_pred, average=avg),
        f1_score(y_pred.argmax(axis=1), np.argmax(y_label, axis=1), average=avg), f1_score(y_pred.argmax(axis=1), np.argmax(y_label, axis=1), average=None)[2])
...
def getSurvTestPats_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_rnaseq/', model='pathomic_fusion', split='test', use_rnaseq=True, agg_type='Hazard_mean'):
    pats = {}
    ...
    # for k in range(1,8):
        pred = pickle.load(open(ckpt_name+'/%s/%s_%d%spred_%s.pkl' % (model, model, k, use_patch, split), 'rb'))   
        # print(pred)
        surv_all = pd.DataFrame(np.stack(np.delete(np.array(pred), [5,6,7]))).T
        surv_all.columns = ['Hazard_fuse', 'Hazard_path', 'Hazard_omic', 'Survival months', 'censored', 'Grade']
        # print(surv_all)
        # print("number of censored:", np.sum(np.array(surv_all['censored'])))
        data_cv = pickle.load(open(data_cv_path, 'rb'))
        data_cv_splits = data_cv['cv_splits']
        data_cv_split_k = data_cv_splits[k]
        assert np.all(data_cv_split_k[split]['t'] == pred[3]) 
        all_dataset = data_cv['data_pd'].drop('TCGA ID', axis=1)
        all_dataset_regstrd = all_dataset.loc[data_cv_split_k[split]['x_patname']] # Subset of "all_datasets" (metadata) that is registered with "pred" (predictions)
        assert np.all(np.array(all_dataset_regstrd['Survival months']) == pred[3])
        assert np.all(np.array(all_dataset_regstrd['censored']) == pred[4])
        assert np.all(np.array(all_dataset_regstrd['Grade']) == pred[8])
        all_dataset_regstrd.insert(loc=0, column='Hazard', value = np.array(surv_all['Hazard_fuse']))
        # print(all_dataset_regstrd.shape)
...
def getPredAggSurv_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_rnaseq/', model='pathomic_fusion', 
    ...
    if 'cox' in model:
        ...
        if model == "pathomic_late_0.01CRD":
            for k in range(1,16):
            # for k in range(1,8):
                results_file = ckpt_name+'/%s/%s_%d%spred_%s.pkl' % (model, model, k, use_patch, split)
                # print("loading results from:", results_file)
                pred = pickle.load(open(results_file, 'rb'))    
                # print(pred)
                surv_all = pd.DataFrame(np.stack(np.delete(np.array(pred), [5, 6, 7]))).T
                surv_all.columns = ['Hazard_fuse', 'Hazard_path', 'Hazard_omic', 'Survival months', 'censored', 'Grade']
                # print(surv_all)
                ...
                data_cv_split_k = data_cv_splits[k]
                # print(pred[3].shape)
                # print(data_cv_split_k[split]['t'].shape)
                assert np.all(data_cv_split_k[split]['t'] == pred[3]) 
                all_dataset = data_cv['data_pd'].drop('TCGA ID', axis=1)
                all_dataset_regstrd = all_dataset.loc[data_cv_split_k[split]['x_patname']] # Subset of "all_datasets" (metadata) that is registered with "pred" (predictions)
                assert np.all(np.array(all_dataset_regstrd['Survival months']) == pred[3])
                ...
                for which_net in ['fuse', 'path', 'omic']:
                # for which_net in ['path']:
                    all_dataset_regstrd.insert(loc=0, column='Hazard', value = np.array(surv_all['Hazard_' + which_net]))
                    hazard_agg = all_dataset_regstrd.groupby('TCGA ID').agg({'Hazard': ['mean', 'median', max]})
                    hazard_agg.columns = ["_".join(x) for x in hazard_agg.columns.ravel()]
                    # print(hazard_agg)
                    hazard_agg = hazard_agg[[agg_type]]
                    ...
                    cin = CIndex_lifeline(all_dataset_hazard['Hazard'], \
                        all_dataset_hazard['censored'], all_dataset_hazard['Survival months'])
                    results.append(cin)
                    del all_dataset_regstrd['Hazard']
            results = np.reshape(np.array(results), (15, 3)).T
            ...
            # for k in range(1,2):
                ...
                data_cv_splits = data_cv['cv_splits']
                data_cv_split_k = data_cv_splits[k]
                # print(data_cv_split_k[split]['t'], pred[1])
                # compare the ground truth for survival time and make sure Data is correctly registered
                ...
                # print(test_pats[k])
                # test_pats[k] denotes the ID of test patients in the split k.
                # align the sample ID for different experiments, 
                # make sure the same test set is used in different experiments.
                all_dataset_hazard = all_dataset_hazard.loc[test_pats[k]]
                # print(all_dataset_hazard.shape)

                cin = CIndex_lifeline(all_dataset_hazard['Hazard'], all_dataset_hazard['censored'], all_dataset_hazard['Survival months'])
                results.append(cin)
            results = np.reshape(np.array(results), (1, 15))
