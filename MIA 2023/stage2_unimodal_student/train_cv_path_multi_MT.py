...
# from data_loaders_new import pathomic_dataloader, pathomic_patches_dataloader
from options_new import parse_args
# from train_test_path_multi_distill_0218 import train, test
from train_test_path_multi_distill import train, test
...
if not os.path.exists(os.path.join(opt.checkpoints_dir, opt.model_name)): os.makedirs(os.path.join(opt.checkpoints_dir, opt.model_name))
...
rocauc_path_all, ap_path_all, f1_micro_path_all, f1_gradeIV_path_all, \
	f1_macro_all, recall_macro_all, precision_macro_all, kappa_all, mcc_all = [], [], [], [], [], [], [], [], []
best_auc_all, best_ap_all, best_f1_all, best_f1_gradeIV_all, \
	best_f1_macro_all, best_recall_macro_all, best_prec_macro_all, best_kappa_all, best_mcc_all = [], [], [], [], [], [], [], [], []
avg_auc_all, avg_ap_all, avg_f1_all, avg_f1_gradeIV_all, \
	avg_f1_macro_all, avg_recall_macro_all, avg_prec_macro_all, avg_kappa_all, avg_mcc_all = [], [], [], [], [], [], [], [], []
...
for k, data in data_cv_splits.items():
	if k > 0:
		...
		train_loader, train_class_idx, test_loader, n_data = pathomic_dataloader(opt, data)
		...
		fix_model, model, optimizer, metric_logger, best_test_metrics, avg_test_metrics = train(
			opt, train_loader, train_class_idx, n_data, test_loader, test_loader_patches, device, k)
		...
		if opt.task == 'surv':
			...
			[rocauc_path, ap_path, f1_micro_path, f1_gradeIV_path, \
				f1_macro_path, recall_macro_path, precision_macro_path, kappa_path, mcc_path] = all_grad_metrics
			[best_auc, best_ap, best_f1, best_f1_gradeIV, \
				best_f1_macro, best_recall_macro, best_prec_macro, best_kappa, best_mcc] = best_test_metrics
			[avg_auc, avg_ap, avg_f1, avg_f1_gradeIV, \
				avg_f1_macro, avg_recall_macro, avg_prec_macro, avg_kappa, avg_mcc] = avg_test_metrics
			...
			f1_macro_all.append(f1_macro_path)
			recall_macro_all.append(recall_macro_path)
			precision_macro_all.append(precision_macro_path)
			kappa_all.append(kappa_path)
			mcc_all.append(mcc_path)
			...
			best_f1_macro_all.append(best_f1_macro)
			best_recall_macro_all.append(best_recall_macro)
			best_prec_macro_all.append(best_prec_macro)
			best_kappa_all.append(best_kappa)
			best_mcc_all.append(best_mcc)

			avg_auc_all.append(avg_auc)
			avg_ap_all.append(avg_ap)
			avg_f1_all.append(avg_f1)
			avg_f1_gradeIV_all.append(avg_f1_gradeIV)
			avg_f1_macro_all.append(avg_f1_macro)
			avg_recall_macro_all.append(avg_recall_macro)
			avg_prec_macro_all.append(avg_prec_macro)
			avg_kappa_all.append(avg_kappa)
			avg_mcc_all.append(avg_mcc)
		...
		save_path = os.path.join(opt.checkpoints_dir, opt.model_name, '%s_%d.pt' % (opt.model_name, k))
		torch.save({
			'split':k,
			'opt': opt,
		...
		if opt.task == 'surv':
			...
			print("[Pathology CNN Final] AUC: %.4f +/- %.4f, AP: %.4f +/- %.4f, F1_micro: %.4f +/- %.4f" % (
			...
			print("[Pathology CNN Final] F1-macro: %.4f +/- %.4f, Recall_macro: %.4f +/- %.4f, Prec_macro: %.4f +/- %.4f" % (
                np.array(f1_macro_all).mean(), np.array(f1_macro_all).std(), np.array(recall_macro_all).mean(), \
                np.array(recall_macro_all).std(), np.array(precision_macro_all).mean(), np.array(precision_macro_all).std()))
			print("[Pathology CNN Final] F1_GradeIV: %.4f +/- %.4f, Kappa: %.4f +/- %.4f, MCC: %.4f +/- %.4f" % (
                np.array(f1_gradeIV_path_all).mean(), np.array(f1_gradeIV_path_all).std(), np.array(kappa_all).mean(), \
				np.array(kappa_all).std(), np.array(mcc_all).mean(), np.array(mcc_all).std()))
			

			print("[Pathology CNN last3Avg] AUC: %.4f +/- %.4f, AP: %.4f +/- %.4f, F1_micro: %.4f +/- %.4f" % (
			...
			print("[Pathology CNN last3Avg] F1-macro: %.4f +/- %.4f, Recall_macro: %.4f +/- %.4f, Prec_macro: %.4f +/- %.4f" % (
                np.array(avg_f1_macro_all).mean(), np.array(avg_f1_macro_all).std(), np.array(avg_recall_macro_all).mean(), \
                np.array(avg_recall_macro_all).std(), np.array(avg_prec_macro_all).mean(), np.array(avg_prec_macro_all).std()))
			print("[Pathology CNN last3Avg] F1_GradeIV: %.4f +/- %.4f, Kappa: %.4f +/- %.4f, MCC: %.4f +/- %.4f" % (
                np.array(avg_f1_gradeIV_all).mean(), np.array(avg_f1_gradeIV_all).std(), np.array(avg_kappa_all).mean(), \
				np.array(avg_kappa_all).std(), np.array(avg_mcc_all).mean(), np.array(avg_mcc_all).std()))


			print("[Pathology CNN Best] AUC: %.4f +/- %.4f, AP: %.4f +/- %.4f, F1_micro: %.4f +/- %.4f" % (
			...
			print("[Pathology CNN Best] F1-macro: %.4f +/- %.4f, Recall_macro: %.4f +/- %.4f, Prec_macro: %.4f +/- %.4f" % (
                np.array(best_f1_macro_all).mean(), np.array(best_f1_macro_all).std(), np.array(best_recall_macro_all).mean(), \
                np.array(best_recall_macro_all).std(), np.array(best_prec_macro_all).mean(), np.array(best_prec_macro_all).std()))
			print("[Pathology CNN Best] F1_GradeIV: %.4f +/- %.4f, Kappa: %.4f +/- %.4f, MCC: %.4f +/- %.4f" % (
                np.array(best_f1_gradeIV_all).mean(), np.array(best_f1_gradeIV_all).std(), np.array(best_kappa_all).mean(), \
				np.array(best_kappa_all).std(), np.array(best_mcc_all).mean(), np.array(best_mcc_all).std()))

			# print("[Pathology CNN last3Avg] AUC: %.4f +/- %.4f, AP: %.4f +/- %.4f, F1_micro: %.4f +/- %.4f, F1_GradeIV: %.4f +/- %.4f" % (
            #     np.array(avg_auc_all).mean(), np.array(avg_auc_all).std(), np.array(avg_ap_all).mean(), \
            #     np.array(avg_ap_all).std(), np.array(avg_f1_all).mean(), np.array(avg_f1_all).std(), \
            #     np.array(avg_f1_gradeIV_all).mean(), np.array(avg_f1_gradeIV_all).std())) 
			# print("[Pathology CNN Best] AUC: %.4f +/- %.4f, AP: %.4f +/- %.4f, F1_micro: %.4f +/- %.4f, F1_GradeIV: %.4f +/- %.4f" % (
            #     np.array(best_auc_all).mean(), np.array(best_auc_all).std(), np.array(best_ap_all).mean(), \
            #     np.array(best_ap_all).std(), np.array(best_f1_all).mean(), np.array(best_f1_all).std(), \
