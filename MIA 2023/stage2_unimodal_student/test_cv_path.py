...
for k, data in data_cv_splits.items():
    if k > 0:
        ...
        train_loader, _, test_loader, n_data = pathomic_dataloader(opt, data)
        ...
        # for which_model in ['_best', '']:
            load_path = os.path.join(opt.checkpoints_dir, opt.exp_name, \
                opt.model_name, '%s_%d.pt' % (opt.model_name, k))
            ...
            feats_labels = np.concatenate((feats_path, np.reshape(gt_all+1, (-1,1))), 1)
            print("path features and labels:", feats_labels.shape)
            save_dir = './feature_analysis/' + opt.model_name
            pd.DataFrame(feats_labels).to_csv(save_dir + "/split" + str(k) + "_path_feats_labels.csv", index=False)
