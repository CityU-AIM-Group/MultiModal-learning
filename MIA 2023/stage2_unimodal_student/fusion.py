...
class PolynomialFusion(nn.Module):
    def __init__(self, skip=1, use_bilinear=1, gate1=1, gate2=1, dim1=32, dim2=32, 
        ...
        self.encoder2 = nn.Sequential(nn.Linear((dim1+1)*(dim2+1), mmhid), nn.BatchNorm1d(mmhid),
                                      nn.ReLU(), nn.Dropout(p=dropout_rate))
        self.encoder3 = nn.Sequential(nn.Linear(mmhid+skip_dim, mmhid), nn.BatchNorm1d(mmhid),
                                      nn.ReLU(), nn.Dropout(p=dropout_rate))
    ...
    def forward(self, vec1, vec2):
        ...
        out12 = self.encoder1(out)
        ### 4-order fusion
        out12 = torch.cat((out12, torch.cuda.FloatTensor(out12.shape[0], 1).fill_(1)), 1)
        o1212 = torch.bmm(out12.unsqueeze(2), out12.unsqueeze(1)).flatten(start_dim=1)
        out = self.post_fusion_dropout(o1212)
