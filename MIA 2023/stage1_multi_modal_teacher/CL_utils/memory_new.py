...
class ContrastMemory_v2(nn.Module):
    ...
    def forward(self, epoch, v1, v2, y, idx=None, select_pos_mode="mid"):
        ...
        """
        Compute the relation matrix between each query and all positive candidates from the memory bank (both in teacher and student).
        If the relation is higher in teacher model, the positive candidate is regarded as a meaningful positive pair.
        ...
        if self.select_pos_pairs == True:
            ...
            ### For each sample, select 10 positive pairs with the largest relation difference.
