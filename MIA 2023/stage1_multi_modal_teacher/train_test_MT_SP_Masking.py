...
from sklearn.metrics import average_precision_score, f1_score, roc_auc_score, recall_score, confusion_matrix, precision_score, cohen_kappa_score, matthews_corrcoef
...
from networks_new import define_net, define_decoder_net, define_reg, define_optimizer, define_scheduler
...
def superpixel_attention_mask(opt, optimizer, model, x_path, x_grph, x_omic, sp_mask, grade, device): # sp_mask B, 1, H, W, inputs B, 3, H, W
    
    Path_K = opt.Path_K
    Omic_K = opt.Omic_K
    sp_mask = sp_mask.to(device)
    x_path_adv = x_path.clone().detach()
    x_path_adv = Variable(x_path_adv.data, requires_grad=True).to(device)
    x_omic_adv = x_omic.clone().detach()
    x_omic_adv = Variable(x_omic_adv.data, requires_grad=True).to(device)
    
    x_path_adv_grad = list()
    def grad_hook1(grad):
        x_path_adv_grad.append(grad)
    h1 = x_path_adv.register_hook(grad_hook1)
    x_omic_adv_grad = list()
    def grad_hook2(grad):
        x_omic_adv_grad.append(grad)        
    h2 = x_omic_adv.register_hook(grad_hook2)
    
    model.eval()
    fuse_feat, path_feat, omic_feat, _, _, pred, pred_path, pred_omic, _, _, _ = model(
                x_path=x_path_adv, x_grph=x_grph.to(device), x_omic=x_omic_adv)    

    cost = F.nll_loss(pred, grade)
    model.zero_grad()
    if x_path_adv.grad is not None:
        x_path_adv.grad.data.fill_(0)
    if x_omic_adv.grad is not None:
        x_omic_adv.grad.data.fill_(0)         
    cost.backward()
    x_path_grad =  x_path_adv_grad[0] # B, H
    x_omic_grad =  x_omic_adv_grad[0]    
    h1.remove()
    h2.remove()  
    
    # sp_mask  B, W, H
    B, H, W = sp_mask.size()
    sp_mask_one_hot = F.one_hot(sp_mask).permute(0,3,1,2) # B, N, H, W
    inputs_grad_v = x_path_grad.view(B, 3, -1) # B, 3 HW
    sp_mask_one_hot_v = sp_mask_one_hot.view(B,sp_mask_one_hot.size(1),-1) # B, N HW
    area = sp_mask_one_hot_v.sum(-1) # B, N
    inputs_grad_v = inputs_grad_v.cpu()
    sp_mask_one_hot_v = sp_mask_one_hot_v.cpu()    
    inputs_grad_aggre = torch.bmm(inputs_grad_v, sp_mask_one_hot_v.permute(0,2,1).float()) # B, 3, N
    inputs_grad_aggre = inputs_grad_aggre.to(device)    
    inputs_grad_aggre = inputs_grad_aggre.sum(1) # B, N
    inputs_grad_aggre_mean = inputs_grad_aggre/(area+1e-9) # B, N
    
    # B, K, H, W
    x_path_super_mask = torch.zeros(B, Path_K, H, W).cuda()
    for b in range(B):
        x_path_super_mask[b] = sp_mask_one_hot[b,:,:,:].index_select(0, inputs_grad_aggre_mean[b].topk(Path_K)[1])
    x_path_super_mask = x_path_super_mask.sum(1) # B, H, W
    assert (x_path_super_mask>1).sum()==0
    
    x_omic_super_mask = (x_omic_grad >= x_omic_grad.topk(Omic_K, dim=1)[0][:,-1].view(B, 1)) # B H

    model.train()
    
    
    return x_path_super_mask.float(), x_omic_super_mask.float()
...
def train(opt, train_loader, n_data, test_loader, test_loader_patches, device, k):
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter+opt.niter_decay+1)):
        ...
        loss_pred_KD_masking_epoch = 0
        if epoch >= 15:
            opt.CRD_weight = 0.01
        # ((image, sp_mask, image_ema, sp_mask_ema), 0, single_X_omic, single_e, single_t, single_g)
        for batch_idx, ((x_path, sp_mask, ema_x_path, ema_sp_mask, x_path_m_v1, x_path_m_v2), x_grph, x_omic, censor, survtime, \
            ...
            ema_x_omic = x_omic
            pred_KD_masking_loss = 0.
            if opt.masking:
                if epoch > opt.start_epoch:
                    x_path_super_mask, x_omic_super_mask = superpixel_attention_mask(opt,optimizer, model, x_path, x_grph, x_omic, sp_mask, grade, device) # B, H, W
                    x_path_mask = x_path.to(device) * (1-x_path_super_mask.unsqueeze(1))
                    x_omic_mask = x_omic.to(device) * (1-x_omic_super_mask)

                    _, _, _, _, _, pred_masking_v1, _, _, _, _, _  = model(
                        x_path=x_path_mask, x_grph=x_grph.to(device), x_omic=x_omic.to(device))                     
                                      
                    _, _, _, _, _, pred_masking_v2, _, _, _, _, _  = model(
                        x_path=x_path.to(device), x_grph=x_grph.to(device), x_omic=x_omic_mask)
                                ### teacher model
                    with torch.no_grad():                                                               
                        _, _, _, _, _, ema_pred_m_v1, _, _, _, _, _ = ema_model(
                            x_path=x_path_m_v1.to(device), x_grph=x_grph.to(device), x_omic=x_omic.to(device))

                        _, _, _, _, _, ema_pred_m_v2, _, _, _, _, _ = ema_model(
                            x_path=x_path_m_v2.to(device), x_grph=x_grph.to(device), x_omic=x_omic.to(device))

                    pred_KD_masking_v1 = pred_KD_loss(opt, pred_masking_v1, ema_pred_m_v1)
                    pred_KD_masking_v2 = pred_KD_loss(opt, pred_masking_v2, ema_pred_m_v2)
                        
                    pred_KD_masking_loss = pred_KD_masking_v1 + pred_KD_masking_v2
            ...
            loss = opt.lambda_cox*loss_cox + opt.lambda_nll*loss_nll + opt.lambda_reg*loss_reg \
                 + loss_CRD + loss_pred_KD + loss_SP + pred_KD_masking_loss  # loss_cox = 0 loss_CRD=0 loss_SP=0
            ...
            if pred_KD_masking_loss > 0.0:
                loss_pred_KD_masking_epoch += pred_KD_masking_loss.item() 
        ...
        if opt.measure or epoch == (opt.niter+opt.niter_decay - 1):
            ...
            loss_pred_KD_masking_epoch /= len(train_loader) 
            ...
            if opt.verbose > 0:
                if opt.task == 'surv':
                    ...
                    print('[{}][{:s}]\t\t CRD Loss: {:.4f}'.format(epoch,'Train', loss_CRD_epoch))                    
                    print('[{}][{:s}]\t\t pred KD Loss: {:.4f}'.format(epoch,'Train', loss_KD_epoch))
                    print('[{}][{:s}]\t\t SP Loss: {:.4f}'.format(epoch,'Train', loss_SP_epoch))
                    print('[{}][{:s}]\t\t pred KD masking Loss: {:.4f}'.format(epoch,'Train', loss_pred_KD_masking_epoch))
                    
                                        
                    print('[{}][{:s}]\t\tLoss: {:.4f}, {:s}: {:.4f}'.format(epoch,'Train', loss_path_epoch, 'path accuracy', grad_path_epoch))
                    print('[{}][{:s}]\t\tLoss: {:.4f}, {:s}: {:.4f}'.format(epoch,'Train', loss_omic_epoch, 'omic accuracy', grad_omic_epoch))
                    print('[{}][{:s}]\t\tLoss: {:.4f}, {:s}: {:.4f}\n'.format(epoch,'Train', loss_fuse_epoch, 'fuse accuracy', grad_acc_epoch))

                    print('[{}][{:s}]\t\tLoss: {:.4f}, {:s}: {:.4f}'.format(epoch,'Test', loss_test_path, 'path accuracy', grad_path_test))
                    print('[{}][{:s}]\t\tLoss: {:.4f}, {:s}: {:.4f}'.format(epoch,'Test', loss_test_omic, 'omic accuracy', grad_omic_test))
                    print('[{}][{:s}]\t\tLoss: {:.4f}, {:s}: {:.4f}'.format(epoch,'Test', loss_test_fuse, 'fuse accuracy', grad_acc_test))
...
def test(opt, module_list, model, test_loader, device):
    ...
    for batch_idx, ((x_path, sp_mask), x_grph, x_omic, censor, survtime, grade) in enumerate(test_loader):
    ...
    if opt.task == "grad":
        ...
        rocauc_fuse, ap_fuse, f1_micro_fuse, f1_gradeIV_fuse,f1_macro_fuse, recall_macro_fuse, precision_macro_fuse, kappa_fuse, mcc_fuse = grading_metrics(grad_gt, probs_all)
        rocauc_path, ap_path, f1_micro_path, f1_gradeIV_path,f1_macro_path, recall_macro_path, precision_macro_path, kappa_path, mcc_path = grading_metrics(grad_gt, probs_path)
        rocauc_omic, ap_omic, f1_micro_omic, f1_gradeIV_omic,f1_macro_omic, recall_macro_omic, precision_macro_omic, kappa_omic, mcc_omic = grading_metrics(grad_gt, probs_omic)
        print("Fused branch:", rocauc_fuse, ap_fuse, f1_micro_fuse, f1_gradeIV_fuse,f1_macro_fuse, recall_macro_fuse, precision_macro_fuse, kappa_fuse, mcc_fuse)
        print("Path branch:", rocauc_path, ap_path, f1_micro_path, f1_gradeIV_path,f1_macro_path, recall_macro_path, precision_macro_path, kappa_path, mcc_path)
        print("Omic branch:", rocauc_omic, ap_omic, f1_micro_omic, f1_gradeIV_omic,f1_macro_omic, recall_macro_omic, precision_macro_omic, kappa_omic, mcc_omic)

        all_grad_metrics = [rocauc_fuse, ap_fuse, f1_micro_fuse, f1_gradeIV_fuse,f1_macro_fuse, recall_macro_fuse, precision_macro_fuse, kappa_fuse, mcc_fuse,
                            ...
                            rocauc_omic, ap_omic, f1_micro_omic, f1_gradeIV_omic,f1_macro_omic, recall_macro_omic, precision_macro_omic, kappa_omic, mcc_omic]
    else:
